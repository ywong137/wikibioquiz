...
async function fixAllHints() {
  console.log('🔧 FIXING ALL HINTS - FOCUSED APPROACH');
  console.log(`⏰ Start: ${new Date().toLocaleTimeString()}`);
  
  // Get count of entries to fix
  const countResult = await db.execute(`
    SELECT COUNT(*) as count FROM famous_people 
    WHERE sections IS NOT NULL
  `);
  const totalToFix = countResult.rows[0].count;
  console.log(`📊 Total entries to fix: ${totalToFix}`);
  ...
  while (true) {
    ...
    // Get next batch of entries with sections
    const people = await db
      .select()
      .from(famousPeople)
      .where(isNotNull(famousPeople.sections))
      .limit(20)
      .offset(totalProcessed);
    
    if (people.length === 0) {
      console.log('🎉 ALL HINTS FIXED!');
      break;
    }
    
    console.log(`🔧 BATCH ${batchNumber}: Fixing ${people.length} entries (${totalProcessed + 1}-${totalProcessed + people.length})...`);
    ...
    for (const person of people) {
      try {
        ...
        totalProcessed++;
        console.log(`  ✅ Fixed: ${person.name} → "${properHint}"`);
        
        // Rate limiting to avoid overwhelming OpenAI API
        await new Promise(resolve => setTimeout(resolve, 1000));
        
      } catch (error) {
        console.log(`❌ Error fixing ${person.name}: ${error}`);
        totalProcessed++; // Still count to avoid infinite loop
      }
    }
    ...
    console.log(`📊 PROGRESS: ${totalProcessed}/${totalToFix} (${((totalProcessed/totalToFix)*100).toFixed(1)}%) - ${rate.toFixed(1)} per minute`);
...
fixAllHints().catch(console.error);